...
void Server::run() {
    ...
    while(running.load(std::memory_order_relaxed)) {
        ...
        std::vector<std::unique_ptr<Packet>> packets = networking.pollAndSend();
        ...
        while(world.has_next_event()) {
            ...
            if(event.type == WorldEvent::UNIT_UPDATE) {
                broadcast("world unit " + world.units[event.identity].toString());
            } else if(event.type == WorldEvent::COMMAND) {
                broadcast("world command " + world.unit_command_string(event.command, event.command_ids));
            } else if(event.type == WorldEvent::DEAD_UNIT) {
                broadcast("world unit " + world.units[event.identity].toString());
            } else if(event.type == WorldEvent::PROJECTILE) {
                broadcast("world projectile " + world.projectiles[event.identity].toString());
            }
        }
    }
...
void Server::send(User& user, const std::string& message) {
    networking.send(std::make_unique<Packet>(Packet(message, user.address)));
}
