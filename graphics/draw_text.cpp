...
DrawText::DrawText() : vao(-1), vbo(-1), ftLibrary() {
    ...
    try {
        ...
        assert(posAttrib >= 0);
        assert(texAttrib >= 0);
        assert(!vsource.empty());
        assert(!fsource.empty());
...
void DrawText::initFont(const std::string& name, int size) {
    std::string file = FONT_BASE_PATH + name + ".ttf";
